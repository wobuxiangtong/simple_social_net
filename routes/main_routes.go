...
func Profile(ctx iris.Context) {
	...
	// USER DETAILS
	db.QueryRow("SELECT COUNT(id) AS userCount, id AS userID, username, email, bio FROM users WHERE id=? group by id, username, email, bio", user).Scan(&userCount, &userID, &username, &email, &bio)
...
func ViewPost(ctx iris.Context) {
	...
	// post details
	db.QueryRow("SELECT COUNT(postID) AS postCount, postID, title, content, createdBy, createdAt FROM posts WHERE postID=? group by postID, title, content, createdBy, createdAt", param).Scan(&postCount, &postID, &title, &content, &createdBy, &createdAt)
